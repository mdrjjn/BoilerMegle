...
io.sockets.on('connection', function(socket) {
    ...
    if(!rooms[socket.room]) rooms[socket.room] = socket.room
	...
	socket.on('vote', function(data) {
		...
		switch (data) {
			case 'A':
        io.sockets.in(socket.room).emit('updatechat', 'SERVER-VOTES', {'vote' : 'A'})
				break;
			case 'B':
        io.sockets.in(socket.room).emit('updatechat', 'SERVER-VOTES', {'vote' : 'B'})
				break;
			case 'C':
        io.sockets.in(socket.room).emit('updatechat', 'SERVER-VOTES', {'vote' : 'C'})
				break;
			case 'D':
        io.sockets.in(socket.room).emit('updatechat', 'SERVER-VOTES', {'vote' : 'D'})
				break;
		}
	})
